"""What the research server publishes about itself, and how it refuses an uncredentialed call."""
...
from veupathdb_mcp.research.auth import ServiceTokenVerifier
from veupathdb_mcp.research.settings import get_research_settings

RESOURCE_NAME = "veupathdb-research-mcp"
DEFAULT_MCP_PATH = "/mcp"

# A service secret is issued by the deployment that runs this server, so the
# document names the server itself as the authority that holds the secrets.
_NO_BASE_URL = (
    "RESEARCH_MCP_BASE_URL must name the public URL of veupathdb-research-mcp."
)


def _resource_url(mcp_path: str) -> AnyHttpUrl:
    """The protected resource, which RFC 9728 requires to be the MCP endpoint."""
    base = get_research_settings().base_url.strip()
    if not base:
...
def protected_resource_routes(mcp_path: str = DEFAULT_MCP_PATH) -> list[Route]:
    """The RFC 9728 document. This server issues its own secrets, so it is the authority."""
    return create_protected_resource_routes(
        resource_url=_resource_url(mcp_path),
        authorization_servers=[AnyHttpUrl(get_research_settings().base_url)],
...
def guarded(app: ASGIApp, mcp_path: str = DEFAULT_MCP_PATH) -> ASGIApp:
    ...
    return AuthenticationMiddleware(
        ...
        backend=BearerAuthBackend(ServiceTokenVerifier()),
    )
